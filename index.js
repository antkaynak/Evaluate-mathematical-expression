...
const infixToPostfix = (num) => {
    ...
    for (let i = 0; i < infix.length; i++) {
        if (returnPrecedence(infix[i]) == 0) {
            ...
            } else if (returnPrecedence(infix[i]) > returnPrecedence(tempPostfix[tempPostfix.length - 1])) {
                tempPostfix.push(infix[i]);
            } else if (returnPrecedence(infix[i]) <= returnPrecedence(tempPostfix[tempPostfix.length - 1])) {
                if (infix[i] == ")") {
                    while (tempPostfix[tempPostfix.length - 1] != "(" && tempPostfix.length > 0) {
                        postfix.push(tempPostfix.pop());
                    }
                    tempPostfix.pop();
                } else {
                    while (returnPrecedence(infix[i]) <= returnPrecedence(tempPostfix[tempPostfix.length - 1]) && tempPostfix.length > 0 && tempPostfix[tempPostfix.length - 1] != "(") {
                        postfix.push(tempPostfix.pop());
                    }
                    tempPostfix.push(infix[i]);
                }
...
const formatInput = (s) => {
    ...
    for (let k = 0; k < result.length; k++) {
        if (result[k] !== "-" && result[k] !== "(") {
            ...
            if (k + 2 !== result.length && !isNaN(Number(result[k + 1])) && isNaN(Number(result[k + 2])) && result[k + 2] !== ")") {
                result[k + 1] = "-" + result[k + 1];
                result[k] = undefined;
            }
        ...
        else if (minusFlag && result[k] === "-" && result[k + 1] !== "(" && k + 1 !== result.length) {
            result[k + 1] = "-" + result[k + 1];
            result[k] = undefined;
        }
    }
